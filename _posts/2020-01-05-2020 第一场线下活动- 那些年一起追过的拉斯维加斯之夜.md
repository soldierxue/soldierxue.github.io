...
其次是**开销即用的机器学习SaaS服务**，机器学习或深度学习只有产生业务价值才是客户的最终诉求，不断发展来自亚马逊电商20多年沉淀的智能算法服务，可以直接嵌入到客户业务工作流中，比如预测（Amazon Forecast）和推荐（Amazon Personalize），欺诈识别（Amazon Fraud Detector），翻译服务（Amazon Translate），图像和视频识别和处理（AMAZON REKOGNITION），文本分析（AMAZON COMPREHEND），文档 OCR 文字和表格提取（AMAZON TEXTRACT），文本转语音（AMAZON POLLY），语音转文本即转录（AMAZON TRANSCRIBE）等等。
